...
3. Run `npm run dna:verify:private` once Phase 1 implements it.
4. If no verifier exists for a new source, write a local read-only analysis that emits only approved aggregate names and values. Review its output before preserving any result.
5. Compare:
...
- Update the shared fact record once, then confirm English, Danish, v2, v3, and v4 consume it.
...
- Run localization parity and evidence tests.
