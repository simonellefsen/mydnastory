...
export function AutosomalBeyond({ profile }: { profile: Profile }) {
  ...
  const het = beyond.heterozygosity;
  const roh = beyond.roh;
    ...
    <div className="mt-16">
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-2">
        <article className="rounded-3xl border border-white/10 p-6">
          ...
          <p className="mt-3 text-sm leading-relaxed text-muted">{s.beyond.hetCopy(profile.firstName, hetPct)}</p>
        </article>
        <article className="rounded-3xl border border-white/10 p-6">
          <p className="kicker">{s.beyond.rohTitle}</p>
          <p className="mt-3 font-display text-3xl">{roh.length}</p>
          <p className="mt-3 text-sm leading-relaxed text-muted">{s.beyond.rohCopy(String(roh.length), longestRoh)}</p>
        </article>
      </div>
    </div>
  );
